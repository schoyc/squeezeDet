...
class vkitti(imdb):
  def __init__(self, image_set, data_path, mc):
    ...
    self._image_path = os.path.join(self._data_root_path, image_set, 'image_2')
  ...
  def evaluate_detections(self, eval_dir, global_step, all_boxes):
    ...
    cmd = self._eval_tool + ' ' \
          + os.path.join(self._data_root_path, self._image_set) + ' ' \
  ...
  def do_detection_analysis_in_eval(self, eval_dir, global_step):
    ...
    det_error_dir = os.path.join(
      eval_dir, 'detection_files_{:s}'.format(self._image_set),
        'error_analysis')
