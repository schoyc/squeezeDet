...
class SqueezeDetAdv(ModelSkeleton):
  def __init__(self, mc, gpu_id=0):
    with tf.device('/gpu:{}'.format(gpu_id)):
      ModelSkeleton.__init__(self, mc)

      self._add_adv_inputs()
      self._add_forward_graph()
      self._add_interpretation_graph()
      self._add_loss_graph()
      # self._add_train_graph() Don't need to train for adv gen
      # self._add_viz_graph()

  def _add_adv_inputs(self):
      mc = self.mc
      orig_image_input = self.image_input

      # image input variable (for adv)
      self.image_input = tf.Variable(
          tf.zeros([mc.BATCH_SIZE, mc.IMAGE_HEIGHT, mc.IMAGE_WIDTH, 3], dtype=tf.float32),
          name='image_input'
      )

      # image input placeholder
      self.adv_image_input = orig_image_input
