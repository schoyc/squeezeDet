  ...
  with tf.Graph().as_default():
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
      ...
      for f in glob.iglob(FLAGS.input_path):
        ...
        # ph_image_input = tf.placeholder(
        #     tf.float32, [mc.BATCH_SIZE, mc.IMAGE_HEIGHT, mc.IMAGE_WIDTH, 3],
        #     name='image_input'
        # )
        # # Assign
        # assign_image = tf.assign(model.ph_image_input, ph_image_input)
        # print("Ran assignment")
        # sess.run(assign_image, feed_dict={ph_image_input: [input_image]})
        ...
        # Shapes
        print("boxes", model.det_boxes.shape, det_boxes.shape)
        print("probs", model.det_probs.shape, det_probs.shape)
        print("class", model.det_class, det_class.shape)
